	...
	<div class="app-contain" :style='{"padding":"0px 0","margin":"20px auto 40px","alignItems":"flex-start","borderRadius":"0px","flexWrap":"wrap","background":"none","display":"flex","width":"86%","position":"relative","justifyContent":"space-between"}'>
		...
		<div class="detail_view">
			...
			<div class="info_view">
				...
				<div class="info_item">
					<div class="info_label">开始日期</div>
					<div  class="info_text" >{{detail.kaishiriqi}}</div>
				</div>
				<div class="info_item">
					<div class="info_label">结束日期</div>
					<div  class="info_text" >{{detail.jieshuriqi}}</div>
				</div>
				<div class="info_item">
					<div class="info_label">计划时间</div>
				...
				<div class="btn_view">
					<el-button class="cross_btn" v-if="btnFrontAuth(tableName,'进度')" @click="jihuajinduonAcross('进度','jihuajindu','','','')" type="warning">进度</el-button>
					<el-button class="edit_btn" v-if="centerType&&btnAuth('xuexijihua','修改')" type="primary" @click="editClick">修改</el-button>
					<el-button class="del_btn" v-if="centerType&&btnAuth('xuexijihua','删除')" type="danger" @click="delClick">删除</el-button>
				</div>
			</div>
		</div>
		<el-tabs type="border-card" v-model="activeName" class="tabs_view">
			<el-tab-pane label="计划内容" name="first">
				<div v-html="detail.jihuaneirong"></div>
			</el-tab-pane>
		</el-tabs>
	</div>
</template>
<script setup>
	...
	const jihuajinduonAcross = (btnType,table,crossOptAudit,statusColumnName,tips,statusColumnValue) => {
