...
<template>
	<div>
			...
			<el-table
				...
				<el-table-column label="操作" width="300" :resizable='true' :sortable='true' align="left" header-align="center">
					<template #default="scope">
						...
						<el-button v-if="btnAuth('xuexiziyuan','学习')" type="success" @click="xuexixinxiCrossAddOrUpdateHandler(scope.row,'cross','','','')">学习</el-button>
		...
		<xuexixinxiFormModel ref="xuexixinxiFormModelRef" @formModelChange="formModelChange"></xuexixinxiFormModel>
	</div>
</template>
<script setup>
	...
	import xuexixinxiFormModel from '@/views/xuexixinxi/formModel'
	const xuexixinxiFormModelRef = ref(null)
    const xuexixinxiCrossAddOrUpdateHandler = (row,type,crossOptAudit,statusColumnName,tips,statusColumnValue) => {
		...
		nextTick(()=>{
			xuexixinxiFormModelRef.value.init(row.id,'cross','学习',row,'xuexiziyuan',statusColumnName,tips,statusColumnValue)
		})
    }
	//初始化
	const init = () => {
