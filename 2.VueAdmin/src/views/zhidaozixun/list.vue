	...
	<div>
		<div class="app-contain">
			<div class="list_search_view">
					...
					<div class="search_view">
						...
						<div class="search_box">
							<el-select
								class="search_sel"
								clearable
								v-model="searchQuery.daoshizhanghao" 
								placeholder="导师账号"
								>
								<el-option v-for="item in daoshizhanghaoLists" :label="item" :value="item"></el-option>
							</el-select>
						</div>
					</div>
				...
				<br>
				<div class="btn_view">
					<el-button type="success" @click="addClick" v-if="btnAuth('zhidaozixun','新增')">新增</el-button>
					<el-button  v-if=" btnAuth('zhidaozixun','查看')" type="info"  :disabled="selRows.length==1?false:true" @click="infoClick(null)">详情</el-button>
					<el-button type="primary" :disabled="selRows.length==1?false:true" @click="editClick" v-if=" btnAuth('zhidaozixun','修改')">修改</el-button>
					<el-button type="danger" :disabled="selRows.length?false:true" @click="delClick(null)"  v-if="btnAuth('zhidaozixun','删除')">删除</el-button>
				</div>
			</div>
			<br>
			<el-table
				...
				<el-table-column label="审核" v-if="btnAuth('zhidaozixun','审核')" :resizable='true' :sortable='true' align="left" header-align="center">
					<template #default="scope">
				...
				<el-table-column label="操作" width="300" :resizable='true' :sortable='true' align="left" header-align="center">
					<template #default="scope">
						<el-button type="info" v-if=" btnAuth('zhidaozixun','查看')" @click="infoClick(scope.row.id)">详情</el-button>
					</template>
				</el-table-column>
			</el-table>
			<el-pagination 
...
<script setup>
	...
	const getList = () => {
		...
		params['sort'] = 'id'
		params['order'] = 'desc'
		if(searchQuery.value.daoshizhanghao&&searchQuery.value.daoshizhanghao!=''){
			params['daoshizhanghao'] = searchQuery.value.daoshizhanghao
		}
		context?.$http({
			url: `${tableName}/page`,
			method: 'get',
	...
	//搜索
	const daoshizhanghaoLists = ref([])
	const getdaoshizhanghaoLists = () => {
		context?.$http({
			url: 'option/daoshi/daoshizhanghao',
			method:'get'
		}).then(res=>{
			daoshizhanghaoLists.value = res.data.data
		})
	}
	getdaoshizhanghaoLists()
	const searchClick = () => {
	...
	const approvalClick = (row) => {
		let params = {
			id:row.id,
			daoshizhanghao: row.daoshizhanghao,
